...
print_debug = True
def log(*args): #Command line argument thingy
    if print_debug:
        print(*args)
        return
    return
...
async def payload():
    ...
    await multitask(arm.run_angle(400,15),db.turn(90, then=Stop.HOLD))
    db.settings(straight_speed=400, turn_rate=175)
    await db.straight(508, then=Stop.NONE)
    db.settings(straight_speed=50)
    await multitask(db.straight(40, then=Stop.HOLD), payloadsub(), arm.run_angle(500,-10))    
    ...
    await multitask(arm.run_angle(-100,120, then=Stop.HOLD), db.straight(-635, then=Stop.HOLD))
    arm.control.limits(acceleration=2000)    
...
### To get the position in the domain of 0 < theta < 360, you have to use the modulo operator, which gives the remainader.
## Another suggestion is to add an inbuilt timer, and then add a comment to every main end of a section. (USE LOG SO YOU CAN DISABLE AND ENABLE)
...
async def surpriseprediction():
    ...
    await stupidlinesquare(100)
    await db.arc(radius=60, angle=90)
    await line(300, 250, 1)
async def test():
    db.settings(straight_speed=defaultSpeed)
