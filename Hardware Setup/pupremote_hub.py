...
class PUPRemoteHub(PUPRemote):
    ...
    async def call_async(self, mode_name: str, *argv, wait_ms=0):
        """
        Asynchronous version of `call()`. Call a remote function on the sensor side
        using async/await in Pybricks.
        """
        ...
        # Ensure sensor/device is connected
        try:
            await self.pup_device.read(mode)  # ✅ Await the read operation
        except:
            print("Check wiring and remote script. Unable to connect on ", self.port)
            return ()  # ✅ Return an empty tuple instead of None
    
        # Send command if there's a format
        if FROM_HUB_FORMAT in self.commands[mode]: 
            payl = self.encode(size, self.commands[mode][FROM_HUB_FORMAT], *argv)
            formatted_payload = self._int8_to_uint8(tuple(payl + b"\x00" * (size - len(payl))))
    
            self.pup_device.write(mode, formatted_payload)
    
            # Ensure `await wait(wait_ms)` is only used correctly
            if wait_ms > 0:
                await wait(wait_ms)  
    
        # ✅ Fix: Await read operation
        data = await self.pup_device.read(mode)
    
        # ✅ Ensure `data` is valid before proceeding
        if not isinstance(data, (bytes, tuple, list)):
            print("Error: Unexpected data format from read. Data =", data)
            return ()  # ✅ Return an empty tuple instead of causing an error
    
        size = len(data)  # ✅ Now safe because `data` is checked
        raw_data = struct.pack(f'{size}b', *data)  # Convert data correctly
