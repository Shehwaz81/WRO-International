...
async def payloadbackup():
    await wait(600)
    await db.straight(-625, then=Stop.HOLD)
...
async def payload():
    ...
    db.settings(straight_speed=defaultSpeed)
    await multitask(arm.run_angle(-500, 500, then=Stop.HOLD),payloadbackup())
    await db.turn(-90, then=Stop.HOLD)
...
async def flags():
    ...
    await arm.run_angle(500, -230, then=Stop.HOLD)
...
async def rotatebolts():    
    ...
    print(type(desired_bl))
    print(desired_bl, desired_br)
    ...
    for move in sequence:
        count += 1
        if move == "cw":
            await spin.run_angle(500,100)
            await spin.run_angle(500,-10)
        elif move == "ccw":
            await spin.run_angle(500,-100)
            await spin.run_angle(500,10)
        elif move == "180":
            ...
            await multitask(arm.run_angle(500, -500), db.straight(30))
            await db.straight(70)
