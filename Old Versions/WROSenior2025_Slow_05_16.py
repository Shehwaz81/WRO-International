...
async def flags():
    ...
    await db.turn(-90, then=Stop.HOLD)
    await multitask(db.straight(-20, then=Stop.HOLD),arm.run_angle(450, 300, then=Stop.HOLD))
...
#    await db.straight(20)
    ...
    await db.turn(-85)
    await multitask(nosesubtask(), arm.run_angle(400,-475))
...
async def lastbolts():
    ...
    await db.turn(89.5)
    await stupidlinesquare(100)
    await multitask(db.straight(115), arm.run_angle(400,-200))
    await db.straight(-200)
