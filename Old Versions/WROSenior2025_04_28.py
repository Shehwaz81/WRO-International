...
async def payloadbackup():
    await wait(400)
    await db.straight(-645, then=Stop.HOLD)
...
async def flags():
    ...
    await arm.run_angle(600, -218, then=Stop.HOLD)
...
async def engagebolts():
    ...
    await arm.run_angle(300, 320, then=Stop.HOLD)
...
async def rotatebolts():    
    ...
    for move in sequence:
        ...
        if move == "cw":
            ...
            await multitask(db.straight(120), arm.run_angle(500,-440))
            db.settings(straight_speed=defaultSpeed)
            arm.control.limits(acceleration=1000)
            await arm.run_angle(500,300)
            await db.straight(-120)
            await arm.run_angle(300,230)
            await spin.run_angle(500,185)
            await spin.run_angle(500,-5)
            arm.control.limits(acceleration=1600)
            await arm.run_angle(500, -300)
            await multitask(arm.run_angle(500, -230),db.straight(155))
...
async def redgate():
    ...
    await arm.run_angle(400,430)
    await db.straight(-90)
    await arm.run_angle(400,-230)
    await db.straight(-200)
    await db.turn(90)

async def noses():
    await line(250,300, -1)
async def main():
    await payload()
    await collectbolts()
    await flags()
