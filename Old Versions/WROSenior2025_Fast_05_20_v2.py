...
async def payloadsub():
    ...
    if block1id == 1:
        await multitask(spin.run_angle(1200, 109, then=Stop.HOLD), db.straight(38, then=Stop.HOLD))
        await spin.run_angle(800, -19, then=Stop.HOLD)
        payloadbackup = -2
    elif block1id == 3:
        await multitask(spin.run_angle(1200, -107, then=Stop.HOLD), db.straight(45, then=Stop.HOLD))
        await spin.run_angle(800, 17, then=Stop.HOLD)
        payloadbackup = 5
    elif block1id == 4:
        await multitask(spin.run_angle(1200, 199, then=Stop.HOLD), db.straight(40, then=Stop.HOLD))
        await spin.run_angle(800, -19, then=Stop.HOLD)
        payloadbackup = 0
    else:
        await db.straight(43, then=Stop.HOLD)
        payloadbackup = 3

async def payload():
    db.use_gyro(True)
    db.settings(straight_speed=400, turn_rate=90, straight_acceleration=defaultAcceleration)
    ...
    await multitask(arm.run_angle(400,10),db.turn(90, then=Stop.HOLD))
    db.settings(straight_speed=400, turn_rate=175)
    await db.straight(508, then=Stop.NONE)
    db.settings(straight_speed=50)
    await multitask(payloadsub(), arm.run_angle(500,-10))    
    db.settings(straight_speed=defaultSpeed)
    arm.control.limits(acceleration=1800)
    await arm.run_angle(-550,405, then=Stop.HOLD)
    arm.control.limits(acceleration=2000)
    await multitask(arm.run_angle(-100,120, then=Stop.HOLD), db.straight(-635-payloadbackup, then=Stop.HOLD))
    arm.control.limits(acceleration=2000)    
...
async def collectbolts():
    ...
    await db.straight(205, then=Stop.HOLD)
    await arm.run_angle(500, 200, then=Stop.HOLD)
    await db.straight(-340, then=Stop.HOLD)
async def flagdelay():
    await wait(400)
...
async def flags():
    ...
    await arm.run_angle(500, -240, then=Stop.HOLD)

async def alignbolts():
    await db.straight(-80)
    await db.turn(20)
    await db.straight(477)
...
async def rotatebolts():    
    ...
    for move in sequence:
        count += 1
        if move == "cw":
            await spin.run_angle(3000,95)
            await spin.run_angle(3000,-5)
        elif move == "ccw":
            await spin.run_angle(3000,-95)
            await spin.run_angle(3000,5)
        elif move == "180":
            await spin.run_angle(3000,-187)
            await spin.run_angle(3000,7)
        elif move == "br":
            db.settings(straight_speed=120)
            arm.control.limits(acceleration=2300)
            await arm.run_angle(500,-120)
            await multitask(db.straight(120), arm.run_angle(580,-390))
            ...
            await spin.run_angle(3000,185)
            await spin.run_angle(3000,-5)
            await arm.run_angle(500, -300)
            arm.control.limits(acceleration=1600)
    ...
    if isLastBack == 1:
        ...
        arm.control.limits(acceleration=2000)
        await multitask(db.straight(117), arm.run_angle(670,-360))
        arm.control.limits(acceleration=1100)
        db.settings(straight_speed=275)
        await arm.run_angle(500,250)
        await db.straight(-107)
        db.use_gyro(True)
        await db.turn(90)
    db.settings(straight_speed=defaultSpeed)
...
async def redgate():
    ...
    await db.arc(radius=43, angle=90)

async def noses():
    arm.control.limits(acceleration=2000)
    await multitask(arm.run_angle(300,-200),line(170,250, -1))
    db.settings(straight_speed=defaultSpeed, straight_acceleration=650)
    await db.straight(40, then=Stop.NONE)
    await db.arc(angle=21, radius=-125)
    await db.arc(angle=21, radius=80)
    db.settings(straight_speed=100, straight_acceleration=defaultAcceleration)
    await db.straight(74)
    await arm.run_angle(400, 505)
    db.settings(straight_speed=defaultSpeed, straight_acceleration=600)
    await db.arc(radius=-230, angle=-90)
    await db.straight(-505)
    db.settings(straight_speed=defaultSpeed, straight_acceleration=defaultAcceleration)
    await multitask(arm.run_angle(500,15),rmotor.run_angle(140,230))
    await db.straight(245)
    await arm.run_angle(500,-300)
    db.settings(straight_speed=defaultSpeed)
    await multitask(db.straight(-160), arm.run_angle(200,-50))
    db.settings(straight_speed=defaultSpeed)
    await db.turn(-83)
    await multitask(db.straight(170), arm.run_angle(400,-150))
    await db.straight(-40)
    await arm.run_angle(550,190)
    await multitask(db.arc(radius=-100, angle=-135),arm.run_angle(550,10))
async def lastbolts():
    await wallsquare(-200,800)
    await db.straight(82)
    await db.turn(89.5)
    await stupidlinesquare(100)
    await multitask(db.straight(100), arm.run_angle(400,-200))
    await db.straight(-200)
    await arm.run_angle(400,490)
    await db.straight(21)
    await wait(200)
...
async def surpriseprediction():
    db.settings(straight_speed=500, straight_acceleration=850)
    await db.straight(-10)
    await db.turn(-90)
    await db.straight(-40)
