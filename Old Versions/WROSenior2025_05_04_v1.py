...
async def payload():
    ...
    await arm.run_angle(-550,505, then=Stop.HOLD)
    arm.control.limits(acceleration=2000)
    await multitask(arm.run_angle(-400,0, then=Stop.HOLD), db.straight(-635, then=Stop.HOLD))
    arm.control.limits(acceleration=2000)    
    await db.turn(-90, then=Stop.HOLD)
...
async def scantimer():
    ...
    time = 0
    while True:
        if await pr.call_async('blocb') == (0, 0, 0):
            await wait(1)
            time += 1
            if time > 1500:
                block1id = 3
                ...
                print('Scan failed')
                break
        elif await pr.call_async('bloca') == (0, 0, 0):
            await wait(1)
            time += 1
            if time > 1500:
                block1id = 3
...
async def rotatebolts():    
    ...
    if isLastBack == 1:
        ...
        arm.control.limits(acceleration=1000)
        await arm.run_angle(500,-80)
        await multitask(db.straight(130), arm.run_angle(500,-420))
        db.settings(straight_speed=275)
...
async def noses():
    arm.control.limits(acceleration=2000)
    await line(300,300, -1)
    await db.straight(110)
    await arm.run_angle(350, 290)
...
async def test():    
    print(await pr.call_async('bloca'))
run_task(main())