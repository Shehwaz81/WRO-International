...
async def payload():
    db.use_gyro(True)
    db.settings(straight_speed=defaultSpeed, turn_rate=90, straight_acceleration=defaultAcceleration)
    spin.control.limits(acceleration=2000)
    await db.straight(15, then=Stop.HOLD)
    await arm.run_angle(500, 515, then=Stop.HOLD)
    await db.straight(275, then=Stop.NONE)
    ...
    await multitask(arm.run_angle(-400,120, then=Stop.HOLD), db.straight(-635, then=Stop.HOLD))
    arm.control.limits(acceleration=2000)    
...
async def rotatebolts():    
    ...
    if isLastBack == 1:
        ...
        arm.control.limits(acceleration=2000)
        await multitask(db.straight(110), arm.run_angle(650,-360))
        arm.control.limits(acceleration=1100)
        db.settings(straight_speed=275)
...
async def redgate():
    ...
    await db.arc(radius=42, angle=90)

async def noses():
    arm.control.limits(acceleration=2000)
    await multitask(arm.run_angle(300,-200),line(200,250, -1))
    db.settings(straight_speed=defaultSpeed, straight_acceleration=650)
    await db.straight(50, then=Stop.NONE)
    await db.arc(angle=21, radius=-110)
    await db.arc(angle=21, radius=70)
    ...
    await db.straight(-555)
    db.settings(straight_speed=defaultSpeed, straight_acceleration=defaultAcceleration)
    await multitask(arm.run_angle(500,15),rmotor.run_angle(140,230))
    await db.straight(310)
    await arm.run_angle(500,-300)
    db.settings(straight_speed=defaultSpeed)
    await multitask(db.straight(-140), arm.run_angle(200,-50))
    db.settings(straight_speed=defaultSpeed)
    await db.turn(-83)
    await multitask(db.straight(180), arm.run_angle(400,-150))
    await db.straight(-40)
...
async def lastbolts():
    ...
    await db.straight(10)
    await wait(200)
    await multitask(scantimer(), db.straight(5))
    if block1pos < block2pos: #if block1 is on left 
        desired_bl = str(block1id)
        desired_br = str(block2id)
    else: #else if it is on the right
