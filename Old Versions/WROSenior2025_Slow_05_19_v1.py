...
async def payload():
    ...
    await multitask(arm.run_angle(400,5),db.turn(90, then=Stop.HOLD))
...
async def rotatebolts():    
    ...
    for move in sequence:
        count += 1
        if move == "cw":
            await spin.run_angle(2000,95)
            await spin.run_angle(2000,-5)
        elif move == "ccw":
            await spin.run_angle(2000,-95)
            await spin.run_angle(2000,5)
        elif move == "180":
            await spin.run_angle(2000,-185)
            await spin.run_angle(2000,55)
        elif move == "br":
...
async def noses():
    ...
    await db.straight(290)
    await arm.run_angle(500,-200)
    db.settings(straight_speed=200)
    await multitask(db.straight(-140), arm.run_angle(300,-150))
    db.settings(straight_speed=defaultSpeed)
    await db.turn(-85)
    await multitask(db.straight(195), arm.run_angle(400,-150))
    await db.straight(-40)
