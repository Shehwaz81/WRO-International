...
async def payload():
    ...
    await multitask(arm.run_angle(-400,0, then=Stop.HOLD), db.straight(-645, then=Stop.HOLD))
    arm.control.limits(acceleration=2000)    
    await db.turn(-90, then=Stop.HOLD)
...
async def noses():
    ...
    await arm.run_angle(500, 280, then=Stop.COAST)
    await arm.run_angle(500,-5)
    await db.straight(-50)
    await db.turn(30)
