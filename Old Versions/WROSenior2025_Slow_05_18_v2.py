...
async def payload():
    ...
    await multitask(arm.run_angle(-400,115, then=Stop.HOLD), db.straight(-635, then=Stop.HOLD))
    arm.control.limits(acceleration=2000)    
    await db.turn(-90, then=Stop.HOLD)
...
async def redgate():
    ...
    await db.arc(radius=38, angle=90)
...
async def noses():
    ...
    db.settings(straight_speed=defaultSpeed)
    await db.arc(radius=-250, angle=-90)
    ...
    db.settings(straight_speed=defaultSpeed)
    await db.turn(-85)
    await multitask(db.straight(210), arm.run_angle(400,-150))
    await db.straight(-40)
