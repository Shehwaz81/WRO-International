...
async def payloadsub(blockid):
    ...
    if blockid == 1:
        payloadreset = -90
        await multitask(spin.run_angle(1200, 97, then=Stop.HOLD))
        await spin.run_angle(800, -7, then=Stop.HOLD)
    elif blockid == 3:
        payloadreset = 90
...
async def payload():
    db.use_gyro(True)
    db.settings(straight_speed=400, turn_rate=70, turn_acceleration=400)
    spin.control.limits(acceleration=2000)
    await multitask(spin.run_target(1000,0),db.straight(20, then=Stop.HOLD))
    await arm.run_angle(600, 527, then=Stop.HOLD)
    await db.straight(270, then=Stop.NONE)
    await stupidlinesquare(110)
    await db.straight(-0)
    await multitask(arm.run_angle(100,-2),db.turn(90, then=Stop.HOLD))
    db.settings(straight_speed=400, turn_rate=140, straight_acceleration = 1000)
    await db.straight(545, then=Stop.HOLD)
  ...
  #  await payloadsub(block1id)
    ...
    await multitask(arm.run_angle(-70,120, then=Stop.HOLD), db.straight(-605, then=Stop.HOLD, wait=True))
    arm.control.limits(acceleration=2800)    
...
async def collectbolts():
    ...
    await multitask(arm.run_angle(1000, 250, then=Stop.HOLD),db.turn(3))
    await db.straight(-336, then=Stop.HOLD)
async def flagdelay():
    await wait(400)
...
async def flags():
    ...
    await arm.run_angle(240, 225, then=Stop.HOLD)
    db.settings(straight_speed=300)
    await multitask(arm.run_angle(400, -10, then=Stop.HOLD), db.straight(-70, then=Stop.HOLD))
    await db.straight(5)
    db.settings(straight_speed=defaultSpeed)
    normal()
    await arm.run_angle(300, -185, then=Stop.HOLD)

async def alignbolts():
    db.settings(turn_rate=80)
    print("left 90")
    db.settings(straight_acceleration=1400,turn_rate=50,straight_speed=200,turn_acceleration=400)
    await db.turn(-90-db.angle())
    await wait(200)
    print("back up")
    await db.straight(-68, then=Stop.HOLD)
    print("turn 45")
    await db.turn(45)
  #  await wait(150)
    print("straight")
...
async def engagebolts():
    ...
    await arm.run_angle(300, 90, then=Stop.HOLD)
    await db.straight(-40)
    await arm.run_angle(280, 290, then=Stop.HOLD)
    await arm.run_angle(200,-3)
    await spin.run_angle(250,10)
    await spin.run_angle(250,-10)
    await multitask(db.straight(15),spin.run_angle(100,43))
async def scantimer():
   ...
   # await wait(30000)
    ...
    for move in sequence:
        ...
        if count == len(sequence):
            ...
            finalspeed=0
        if move == "cw":
            await spin.run_angle(550-finalspeed,90+lastturn)
         ...
         #   await spin.run_angle(3000,5)
            ...
            await spin.run_angle(550-finalspeed,-180+lastturn)
        #    await spin.run_angle(3000,5)
            ...
            await multitask(db.straight(30),arm.run_angle(130, -120))
            db.settings(straight_speed=120)
...
async def yellowgate():
    db.settings(straight_speed=450,straight_acceleration=400)
    await multitask(db.straight(-468,then=Stop.HOLD),spin.run_angle(500,110))
    normal()
    await db.turn(75)
...
async def redgate():
    ...
    await db.straight(-125, then=Stop.NONE)
    db.drive(-100,0)
    while not await linesens.reflection() <30:
        await wait(0)
    await db.arc(radius=50, angle=85, then=Stop.HOLD)
...
async def nosedelivery():
  ...
  #  await arm.run_angle(500,-10,then=Stop.COAST)
    ...
    db.settings(straight_speed=defaultSpeed)
    await db.turn(-85)
    await arm.run_angle(550,-165)
    await multitask(db.straight(190, then=Stop.HOLD),arm.run_angle(550,-155))
    await db.straight(-40)
...
#    await wait(200)
  ...
  #  await db.turn(-2.5)
    ...
    if desired_bl == config[1] and desired_br == config[0]:
        ...
        await multitask(db.straight(205), arm.run_angle(500,-440), spin_reset_delay(-180))
        db.settings(straight_speed=400)
        await db.straight(-200)
    else:
        await arm.run_angle(500,-500)
...
async def maintime():
    await multitask(test(),start_timer())
    print("time: ",time)
